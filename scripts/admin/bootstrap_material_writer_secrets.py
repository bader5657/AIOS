...
BEGIN
  ...
  IF NOT has_table_privilege('{CANDIDATE_ROLE}','material_receipts','SELECT')
     ...
     OR has_table_privilege('{CANDIDATE_ROLE}','inventory_movements','SELECT,INSERT,UPDATE,DELETE,TRUNCATE')
     OR NOT has_table_privilege('{POSTING_ROLE}','inventory_movements','SELECT')
     OR has_table_privilege('{POSTING_ROLE}','inventory_movements','UPDATE,DELETE,TRUNCATE')
     OR has_table_privilege('{POSTING_ROLE}','material_stock','INSERT,DELETE,TRUNCATE')
  ...
  IF EXISTS (
    ...
    WHERE c.table_schema='{SCHEMA}' AND c.table_name IN ('material_receipts','material_receipt_items','material_stock','inventory_movements')
      AND (has_column_privilege('{CANDIDATE_ROLE}', format('%I.%I',c.table_schema,c.table_name), c.column_name, 'INSERT,UPDATE')
           OR has_column_privilege('{POSTING_ROLE}', format('%I.%I',c.table_schema,c.table_name), c.column_name, 'INSERT,UPDATE'))
      AND NOT (
