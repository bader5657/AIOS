...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema='{SCHEMA}' AND table_name='material_receipts'
      AND column_name='created_by_actor_reference'
  ) THEN
    GRANT INSERT (created_by_actor_reference) ON public.material_receipts TO {CANDIDATE_ROLE};
  END IF;
END $grant_creator$;
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE c.table_schema='{SCHEMA}' AND c.table_name IN ('material_receipts','material_receipt_items','material_stock','inventory_movements')
      ...
      AND NOT (
        ('{CANDIDATE_ROLE}' IS NOT NULL AND c.table_name='material_receipts' AND c.column_name IN ('receipt_id','supplier_name','document_number','document_date','received_at','source_asset_reference','created_by_actor_reference','status','version','confirmed_version','confirmed_at','confirmation_actor_reference','updated_at'))
  ...
  LOOP
    expected := CASE
      WHEN rec.policy='candidate' AND rec.action='INSERT' AND rec.table_name='material_receipts' THEN rec.column_name IN ('receipt_id','supplier_name','document_number','document_date','received_at','source_asset_reference','created_by_actor_reference')
  ...
  IF (SELECT count(*) FROM pg_attribute at JOIN pg_class c ON c.oid=at.attrelid CROSS JOIN LATERAL aclexplode(at.attacl) a JOIN pg_roles r ON r.oid=a.grantee WHERE c.relnamespace='public'::regnamespace AND r.rolname IN ('{CANDIDATE_ROLE}','{POSTING_ROLE}')) <> (59 + (SELECT count(*) FROM information_schema.columns WHERE table_schema='public' AND table_name='material_receipts' AND column_name='created_by_actor_reference'))
     OR EXISTS (SELECT 1 FROM pg_attribute at JOIN pg_class c ON c.oid=at.attrelid CROSS JOIN LATERAL aclexplode(at.attacl) a JOIN pg_roles r ON r.oid=a.grantee WHERE c.relnamespace='public'::regnamespace AND r.rolname IN ('{CANDIDATE_ROLE}','{POSTING_ROLE}') AND (a.is_grantable OR NOT (
       (r.rolname='{CANDIDATE_ROLE}' AND c.relname='material_receipts' AND ((a.privilege_type='INSERT' AND at.attname IN ('receipt_id','supplier_name','document_number','document_date','received_at','source_asset_reference','created_by_actor_reference')) OR (a.privilege_type='UPDATE' AND at.attname IN ('supplier_name','document_number','document_date','received_at','source_asset_reference','status','version','confirmed_version','confirmed_at','confirmation_actor_reference','updated_at'))))
