...
task input transcribed the approval transport SHA-256 incorrectly as
...
| approval-record transport SHA-256 | `266c39426fae0b04dacf009436334dd34d6791368dcad5066a9b2a37b9bd8a57` |
...
The input SHA is over the 1,327 semantic bytes, excluding the one terminal LF.
The approval SHA is over the complete 3,550-byte transport object, including
its one terminal LF. A future installer must receive the exact P4S2-validated
...
3,550-byte length and authoritative transport SHA-256, and enforce the merged
exclusive `not_after_utc` rule against an absolute UTC clock. The approval is
